...
class Recorder:
    def __init__(self, save_dir: pathlib.Path, record_trajectory: bool = True, record_video: bool = True,
                 trajectory_save_interval: int = 10, video_record_interval: int = 10):
        ...
        self.trajectory_df = pd.DataFrame(columns=["episode", "reward", "success", "length"])
        self.writers = None
    ...
    def record_frame(self, episode: int, obs: Observation, force: bool = False):
        if not self.record_video or (episode % self.video_record_interval != 0 and not force):
            return
        if self.writers is None:
            self.init_writer(episode, obs)
        if self.writers is not None:
            for cam_name, img_array in obs.images.items():
                self.writers[cam_name].append_data(img_array[0])  # Assuming batch size of 1
            
    def finish_episode(self, episode: int, reward: float, success: bool, length: int, force: bool = False):
        if self.record_trajectory:
            self.trajectory_df.loc[len(self.trajectory_df)] = [episode, reward, success, length]
